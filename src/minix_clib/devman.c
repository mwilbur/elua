...
DM_DIR* dm_opendir( const char* dirname )
{
  ...
  pdev = dm_list[ pos ];
  if( pdev->p_opendir == NULL )
  {
    return NULL;
  }
  if( ( data = pdev->p_opendir( rest ) ) == NULL )
    return NULL;
...
struct dm_dirent* dm_readdir( DM_DIR *d )
{
  ...
  if( pdev->p_readdir == NULL )
  {
    return NULL;
  }
  return pdev->p_readdir( d->userdata );
}
...
int dm_closedir( DM_DIR *d )
{
  ...
  pdev = dm_list[ d->devid ];
  if( pdev )
    res = pdev->p_closedir( d->userdata );
...
{
  return ( pdev->p_opendir != NULL && pdev->p_readdir != NULL && pdev->p_closedir != NULL );
