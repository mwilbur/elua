...
static int romfs_open_r( const char *path, int flags, int mode )
{
  FS tempfs;
  int i;
...
static int romfs_close_r( int fd )
{
  romfs_close_fd( fd );
  romfs_num_fd --;
  return 0;
...
static _ssize_t romfs_write_r( int fd, const void* ptr, size_t len )
{
  errno = EINVAL;
  return -1;
}

static _ssize_t romfs_read_r( int fd, void* ptr, size_t len )
{
...
// lseek
static off_t romfs_lseek_r( int fd, off_t off, int whence )
{
  FS* pfs = romfs_fd_table + fd;   
  u32 newpos = 0;
...
// opendir
static void* romfs_opendir_r( const char* dname )
{
...
static struct dm_dirent* romfs_readdir_r( void *d )
{
  u32 off = *( u32* )d;
...
// closedir
static int romfs_closedir_r( void *d )
{
  *( u32* )d = 0;
  return 0;
}
