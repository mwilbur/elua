...
static int mmcfs_open_r( const char *path, int flags, int mode )
{
  int fd;
  int mmc_mode;
...
static int mmcfs_close_r( int fd )
{
...
static _ssize_t mmcfs_write_r( int fd, const void* ptr, size_t len )
{
#if _FS_READONLY
  {
    errno = EIO;
...
static _ssize_t mmcfs_read_r( int fd, void* ptr, size_t len )
{
  UINT bytesRead;
...
// lseek
static off_t mmcfs_lseek_r( int fd, off_t off, int whence )
{
  FIL* pFile = mmcfs_fd_table + fd;
  u32 newpos = 0;
...
// opendir
static DIR mmc_dir;
static void* mmcfs_opendir_r( const char* dname )
{
  void* res = NULL;
...
static struct dm_dirent* mmcfs_readdir_r( void *d )
{
  DIR *pdir = ( DIR* )d;
...
// closedir
static int mmcfs_closedir_r( void *d )
{
  return 0;
}
